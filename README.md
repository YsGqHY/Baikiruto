...
- 覆盖 Minecraft `1.12 - 1.21.2` 物品能力
- 内置 Fluxon 脚本执行链路，并支持通过 API 注册其他脚本类型
- 继承 Zaphkiel 的 `ItemStream` 数据流设计精髓
- 面向实战的物品构建、读回、重构与在线更新

## 功能特性

- 跨版本物品配置与生成（含模型、显示、分组、元数据）
- `ItemStream` 运行时数据流（继承自 Zaphkiel）：`build / read / rebuild / update`
- `components` 简化解析：
